...
{-
- Arbitrary LTL norms are compiled into deterministic monitor automata.
- The automaton progresses based on state snapshots and marks violation states.
- Planning is constrained by synchronously composing the planner transition
  system with the norm automaton and pruning violating transitions.
--
- This reduces temporal logic enforcement to state-based admissibility filtering.
   ie:
  LTL formulas are not evaluated by inspecting full traces.
  Instead, they are compiled into deterministic monitor automata.
  The automaton state summarises the relevant history of the execution.
  Norm enforcement then reduces to progressing the automaton state
  after each planner transition and pruning violating states.
-}
...
data TemporalFormula
  ...
  | Always TemporalFormula
  | Eventually TemporalFormula
  | Until TemporalFormula TemporalFormula deriving (Ord, Eq, Show)

evalStateFormula :: Snapshot -> StateFormula -> Bool
evalStateFormula snap (Atom p) =
  Set.member p snap

evalStateFormula snap (NotF f) =
  not (evalStateFormula snap f)

evalStateFormula snap (AndF f g) =
  evalStateFormula snap f && evalStateFormula snap g

evalStateFormula snap (OrF f g) =
  evalStateFormula snap f || evalStateFormula snap g


-- Evaluate atomic Value
evaluateAtom :: Value -> ProblemDef -> State -> Effect Bool
evaluateAtom SafetyFirst Prob{goalCondition,safetyCondition} st = do 
  safetyCondition st 
evaluateAtom TaskCompletion Prob{goalCondition,safetyCondition} st = do 
  goalCondition st
...
data AtomicProp = APSafety | APTaskCompletion deriving (Eq, Ord, Show)

data SafetyNormState = WaitingForSafety | SafetyAchieved | Violated deriving (Show, Eq, Ord)
...
data NormAutomaton q = NormAutomaton
  ...
  , transition   :: Snapshot -> q -> q
  , isViolation  :: q -> Bool
  {-, alphabet     :: [Value]-}   -- relevant propositions
  }

transitionSafetyNorm
  :: Snapshot
  -> SafetyNormState
  -> SafetyNormState
transitionSafetyNorm snap WaitingForSafety
  | Set.member APTaskCompletion snap
    && not (Set.member APSafety snap)
      = Violated

  | Set.member APSafety snap
      = SafetyAchieved

  | otherwise
      = WaitingForSafety

transitionSafetyNorm _ SafetyAchieved = SafetyAchieved
transitionSafetyNorm _ Violated       = Violated

isViolationSafety :: SafetyNormState -> Bool
isViolationSafety Violated = True
isViolationSafety _        = False

type Snapshot = Set.Set AtomicProp -- a snapshot of the state of the world - each entry tells you whether a value is violated or satisfied

snapshotFromState :: ProblemDef -> State -> Effect Snapshot
snapshotFromState prob st = do
  s <- evaluateAtom SafetyFirst prob st
  t <- evaluateAtom TaskCompletion prob st
  return $
    Set.fromList $
      concat
        [ [APSafety        | s]
...
extractRelevantValues :: TemporalFormula -> [AtomicProp]
extractRelevantValues =
  Set.toList . collectSet

collectSet :: TemporalFormula -> Set.Set AtomicProp
collectSet TrueF  = Set.empty
collectSet FalseF = Set.empty

collectSet (StateT sf) =
  collectStateSet sf
...
collectSet (AndT φ ψ) =
  Set.union (collectSet φ) (collectSet ψ)
...
collectSet (Eventually φ) =
  collectSet φ

collectSet (Until φ ψ) =
  Set.union (collectSet φ) (collectSet ψ)

collectStateSet :: StateFormula -> Set.Set AtomicProp

collectStateSet (Atom v) =
  Set.singleton v

collectStateSet (NotF f) =
  collectStateSet f

collectStateSet (AndF f g) =
  Set.union (collectStateSet f)
            (collectStateSet g)

collectStateSet (OrF f g) =
  Set.union (collectStateSet f)
            (collectStateSet g)


progress :: Snapshot -> TemporalFormula -> TemporalFormula
progress snap TrueF  = TrueF
progress snap FalseF = FalseF

progress snap (StateT sf) =
  if evalStateFormula snap sf
     then TrueF
     else FalseF

progress snap (NotT phi) =
  simplify $ NotT (progress snap phi)

progress snap (AndT phi fork') =
  simplify $
    AndT (progress snap phi)
         (progress snap fork')

progress snap (OrT phi fork') =
  simplify $
    OrT (progress snap phi)
        (progress snap fork')

-- Next
progress snap (Next phi) =
  phi

-- Always: G φ ≡ φ ∧ X G φ
progress snap (Always phi) =
  simplify $
    AndT (progress snap phi)
         (Always phi)

-- Eventually: F φ ≡ φ ∨ X F φ
progress snap (Eventually phi) =
  simplify $
    OrT (progress snap phi)
        (Eventually phi)

-- Until: φ U ψ ≡ ψ ∨ (φ ∧ X (φ U ψ))
progress snap (Until phi fork') =
  simplify $
    OrT (progress snap fork')
        (AndT (progress snap phi)
              (Until phi fork'))
...
wrapActionWithNorm aut normVar probDef st action = do
  act  <- action
  snap <- snapshotFromState probDef st
...
compileTemporal phi =
  NormAutomaton
    ...
    , transition   = progress
    , isViolation  = (== FalseF)
    {-, alphabet = [SafetyFirst, TaskCompletion]-}
    }
...
constrainAction
  :: NormAutomaton TemporalFormula      -- norm automaton
  -> Var TemporalFormula               -- mutable norm state
  -> ProblemDef
  -> State
  -> Effect Action        -- original planner action
  -> Effect Action        -- filtered action
constrainAction aut normVar probDef st action = do
...
{-
compileNormAutomatonToPlanner :: NormAutomaton q -> Effect () -> Effect () -}
              


----------------------------------------------------
-- Core Datatypes for Expansion of VFR
----------------------------------------------------
...
instance Eq State where 
  (==) s1 s2 = time s1 == time s2
  (/=) s1 s2 = not $ (==) s1 s2 
instance Ord State where 
  compare s1 s2 = compare (time s1) (time s2) -}
...
data Weight
  = Finite Int -- measurable degree of satisfaction
  | Infinite -- maximally satisfying/overriding
  | Indeterminate
  deriving (Show, Eq)

data Value
  = SafetyFirst
  | TaskCompletion deriving (Ord, Eq, Show)
  -- multiply the importance of v by n

type Time = Int
-- Datatype for both VFR and Planner input
data PropInfoObject = Prop -- This datatype encodes the state which an object is in.
  { object :: Object 
  , location :: Pos
  , clear :: Var Bool
  , fireStatus :: FireVar }
...
data Object = ObjectP Person | Bucket | ObjectB Block | Table | Floor deriving (Show, Eq, Ord)

data Location = OnTable | OnObj Object | InHand | OnFloor deriving (Show, Eq, Ord)
type Pos = Var Location 


data FireStatus = Burning | Safe deriving (Show, Eq, Ord)
type FireVar = Var FireStatus 

data HandStatus = Holding Object | Empty deriving (Show, Eq, Ord)
type HandState = Var HandStatus

data Action = Pickup Object | Douse Object | PutDown Object| StackObject Object Object deriving (Show)
...
problem = do
  ...
  let st = State [aProp,bProp,cProp,tProp,bucketProp,p1Prop,p2Prop]
  let 
    safetY :: State -> Effect Bool 
    safetY st = allM safe (objects st)
      where 
      safe Prop{fireStatus} = do
        f <- readVar fireStatus
        return (f == Safe)
  ...
  let probDef = Prob { goalCondition = \st -> goalCheck st 
                     , safetyCondition = \st -> safetY st }
  ...
  let 
    ...
    clearOf Table = clear tProp
      ...
      -- temporal initial state

  let 
    safetyBeforeTaskAutomaton :: NormAutomaton SafetyNormState
    safetyBeforeTaskAutomaton =
      NormAutomaton
        { initialState = WaitingForSafety
        , transition   = transitionSafetyNorm
        , isViolation  = isViolationSafety
        }
  let normAut = compileTemporal $ StateT (AndF (NotF $ Atom APSafety) (NotF $ Atom APTaskCompletion))

  normVar <- newVar $ initialState normAut



  --------------------------------------------------
  -- Action Effects - With Pre and Post Conditions
  --------------------------------------------------

  let 
    {-withTraceConstraint :: TOperator -> Effect a -> Effect a
    withTraceConstraint op action = do
      tr <- readVar trace
      guard =<< evaluateTrace op probDef tr
      action
    ------------------------------------------------
    -- General Action selector 
    ------------------------------------------------

    actionSelec2Objects :: PropInfoObject -> PropInfoObject -> Effect Action 
    actionSelec2Objects prp1 prp2 = withTraceConstraint (Until SafetyFirst TaskCompletion) $ stack prp1 prp2-}
  ...
  --------------------------------------------------
  let 
    allProps = [aProp,bProp,cProp,p1Prop,p2Prop,tProp,bucketProp]
  ...
  let 
    ...
    actions =
      let raw =
               [ pickUp  p | p <- allProps ]
            ++ [ putDown p | p <- allProps ]
            ++ [ stack p1 p2 | p1 <- allProps, p2 <- allProps ]
            ++ [ douse p | p <- allProps ]
      in
        map (constrainAction normAut normVar probDef st) raw


  --------------------------------------------------
  -- solve searchStrategey actionList goalList
  --------------------------------------------------
